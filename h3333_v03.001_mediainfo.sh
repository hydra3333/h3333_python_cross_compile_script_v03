...
cd ~/Desktop
sudo chmod 777 -R *
#./h3333_cross_compiler_v03.001.py --force --debug --products --dependencies -pl ffmpeg_static_non_free_opencl,x264_multibit
#./h3333_cross_compiler_v03.001.py --force --debug -p x264_multibit
# only use --force if ity's already been fully built, 
# or it breaks if you use it the first time
./h3333_cross_compiler_v03.001.py list -p 2>&1 | tee -a ./0.debug-x264.029.log
./h3333_cross_compiler_v03.001.py list -d 2>&1 | tee -a ./0.debug-x264.029.log
...
echo "# `date` ###################################################################################" >>./0.debug-x264.029.log
echo "# `date` ###################################################################################" >>./0.debug-x264.029.log
./h3333_cross_compiler_v03.001.py --force --debug -d libmediainfo 2>&1 | tee -a ./0.debug-x264.029.log
read -p "After build libmediainfo, press Enter to continue"
./h3333_cross_compiler_v03.001.py --force --debug -p mediainfo 2>&1 | tee -a ./0.debug-x264.029.log
exit_status=$?
