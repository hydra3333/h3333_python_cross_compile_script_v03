...
./h3333_cross_compiler_v03.001.py list -p 2>&1 | tee -a ./0.debug-ff.029.log
./h3333_cross_compiler_v03.001.py list -d 2>&1 | tee -a ./0.debug-ff.029.log
...
echo "# `date` ###################################################################################" >>./0.debug-ff.029.log
echo "# `date` ###################################################################################" >>./0.debug-ff.029.log
./h3333_cross_compiler_v03.001.py --force --debug -p ffmpeg_static_non_free_opencl 2>&1 | tee -a ./0.debug-ff.029.log
exit_status=$?
...
echo "# `date` ###################################################################################" >>./0.debug-ff.029.log
echo "# `date` ###################################################################################" >>./0.debug-ff.029.log
#./h3333_cross_compiler_v03.001.py --force --debug -p ffmpeg_static_non_free 2>&1 | tee -a ./0.debug-ff.029.log
exit_status=$?
echo "exit_status='$exit_status'"
if [ $exit_status -ne 0 ]; then
    echo "Error $exit_status detected"
